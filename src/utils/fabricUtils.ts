...
export const getEffectiveWidth = (obj: fabric.Object): number => {
  if (obj.group) {
    ...
    return Math.round((obj.width || 0) * (obj.scaleX || 1) * (obj.group.scaleX || 1));
  }
...
export const getEffectiveHeight = (obj: fabric.Object): number => {
  if (obj.group) {
    ...
    return Math.round((obj.height || 0) * (obj.scaleY || 1) * (obj.group.scaleY || 1));
  }
  ...
  if (obj.type === 'circle') {
    ...
    if (circleInGroup) {
      ...
      const effectiveRadius =
        (circleInGroup.radius || 0) * (circleInGroup.scaleX || 1) * (group.scaleX || 1);
